classdef getDendriteInfo
    ...
    methods
        ...
        function [x_branch,y_branch] = getBranchPoints_NSWEdit(obj, x_end, y_end)
        ...
        [x_branch, y_branch] = find((bwmorph(obj.image_skeleton, 'branchpoints'))');
        xy_branch = zeros(length(x_end),2);
        
        for i = 1:length(x_end)
            
            distance_endpoints = bwdistgeodesic(obj.image_skeleton, x_end(i), y_end(i), 'quasi-euclidean');
        ...
        function [x_endval, y_endval] = getEndPoints(obj)
            [x_endval, y_endval] = find((bwmorph(obj.image_skeleton, 'endpoints'))');
            % obj.x_end = x_endval;
            % obj.y_end = y_endval;
        end
        
        function [x_perval, y_perval] = getPerimeter(obj)
            [x_perval, y_perval] = find(obj.image_perimeter');
            % obj.x_perim = x_perval;
            % obj.y_perim = y_perval;
        end

        function perim = get.image_perimeter(obj)
            perim = obj.image_perimeter;
        end
    end
end
